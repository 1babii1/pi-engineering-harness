...
assert_allowed "$(tool Bash '{"command":"d+=\"env\""}')" "allows a quoted env literal (assignment)"
assert_allowed "$(tool Bash '{"command":"grep -w \"env\" README.md"}')" "allows grep for the word env in quotes"
assert_allowed "$(tool Bash '{"command":"git commit -m \"env\""}')" "allows env as a quoted commit message"
assert_allowed "$(tool Bash '{"command":"echo \"env\" | jq ."}')" "allows echo of a quoted env"
assert_blocked "$(tool Bash '{"command":"sh -c '"'"'env'"'"'"}')" "blocks sh -c 'env'"
assert_blocked "$(tool Bash '{"command":"bash -c \"sudo env\""}')" "blocks bash -c with a wrapper before env"
assert_allowed "$(tool Bash '{"command":"bash -c \"env FOO=1 dotnet run\""}')" "allows bash -c running env with a command"
# --- commit messages are data; code run inside them, and anything chained after, is still checked ---
assert_allowed "$(tool Bash '{"command":"git commit -m '"'"'document .env handling'"'"'"}')" "allows a single-quoted commit message that mentions .env"
assert_allowed "$(tool Bash '{"command":"git commit -m \"document .env handling\""}')" "allows a double-quoted commit message that mentions .env"
assert_allowed "$(tool Bash '{"command":"git commit -F - <<'"'"'EOF'"'"'\nfix: stop reading .env.local in tests\nEOF"}')" "allows a quoted-heredoc commit message that mentions .env"
assert_allowed "$(tool Bash '{"command":"git -C repo commit -q -m '"'"'ignore .env.* files'"'"'"}')" "allows git -C ... commit -m with a secret glob in the message"
assert_blocked "$(tool Bash '{"command":"git commit -m \"$(cat .env)\""}')" "blocks command substitution reading a secret inside a commit message"
assert_blocked "$(tool Bash '{"command":"git commit -m \"`cat .env`\""}')" "blocks backtick substitution inside a commit message"
assert_blocked "$(tool Bash '{"command":"git commit -F - <<EOF\n$(cat .env)\nEOF"}')" "blocks substitution inside an unquoted heredoc commit message"
assert_blocked "$(tool Bash '{"command":"git commit -m x; cat .env"}')" "blocks a secret read chained after a commit"
assert_blocked "$(tool Bash '{"command":"git commit -m x && cat .env.local"}')" "blocks a secret read chained with && after a commit"
assert_blocked "$(tool Bash '{"command":"git commit -m ok && python3 - <<'"'"'EOF'"'"'\nprint(open(\".env\").read())\nEOF"}')" "blocks a python heredoc after a commit (it is not the commit message)"
assert_blocked "$(tool Bash '{"command":"git add .env"}')" "still blocks git add of a secret file"
